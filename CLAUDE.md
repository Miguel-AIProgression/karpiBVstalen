...
10. **Facturen & creditnota's**: creditnota = `invoices`-rij met `credited_invoice_id` + negatieve bedragen, zelfde STL-reeks; regels snapshot-first uit `invoice_lines`; "dé factuur van een order" = filter `credited_invoice_id IS NULL`. Crediteren alleen via RPC `create_credit_invoice` achter `requireRole(['sales','admin'])`. Verwijderen: alléén niet-gemailde facturen, via `DELETE /api/invoices/[id]` (zelfde rol-gate); een gemailde factuur is onverwijderbaar (BEFORE DELETE-trigger `invoices_no_delete_when_sent`, ook voor service-role) — crediteer in plaats daarvan. Menu-item **Facturatie** (`/facturatie`) is het facturen-werkblad (debet + credit, filters, AFAS-.xlsx-export in RugFlow-verkoopoverzichtformaat, 1 rij per factuur). Mailen: ontvanger zichtbaar/kiesbaar (`to`-override, `invoices.sent_to`); geschiedenis per factuur in `invoice_events`. AFAS-export (17-07): creditnota's zijn los selecteerbaar en gaan mee in de download (alleen vervangen facturen blijven uitgesloten), en 0%-facturen naar een land buiten de EU boeken op tegenrekening 8019/btw-code 33 i.p.v. 8018/34. **Formaat = écht .xlsx (17-07, niet meer CSV)**: de oude ;-CSV had datums als tekst ("17-07-2026"); Excel parste die bij openen volgens de Windows-regio-instelling en toonde/bewaarde ze zonder voorloopnul ("17-7-2026"), wat bij opslaan-als-CSV verminkte datums in AFAS opleverde (boekhouder-klacht). Nu echte datumcellen met geforceerd celformaat `dd-mm-yyyy` (`src/lib/afas-xlsx.ts`, `buildAfasXlsx`) — niet meer regio-afhankelijk. Details: `docs/architecture/database.md` §Facturatie.
