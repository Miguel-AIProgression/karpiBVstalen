...
- **Werkblad `/facturatie`:** facturenlijst (debet + credit) met filters/sortering + AFAS-.xlsx-export (verhuisd van de orders-pagina; alleen debetfacturen selecteerbaar). Lijst-query geplafonneerd op 2000 rijen, nieuwste eerst.
- **AFAS-.xlsx-formaat (13-07, feedback Nando + zijn voorbeeldbestand `2026-07-09.csv`; 17-07 omgezet van CSV naar een écht .xlsx-bestand):** één werkblad, één rij per factuur, exact het formaat dat de boekhouding voor de RugFlow-facturen krijgt: `Debiteur;Naam1;Naam2;Adres;Postcode;Woonplaats;Land;Ordernummer;Klant ref;Factuurnr;Datum;Verv.datum;Bedrag ex;BTW bedrag;Totaal;Tegenrekening;BTW`. Omschrijving + oude letter-BTW-code (0/L/H) vervallen. Naam2 blijft leeg (geen inkoopgroepen in de stalen-app); Nederland → lege Land-kolom (buurlanden genormaliseerd, onbekend land letterlijk); vervaldatum = factuurdatum + `company_settings.payment_days`. **Datum/Verv.datum zijn échte datumcellen met geforceerd celformaat `dd-mm-yyyy`** (niet meer tekst — de oude CSV liet Excel de datum regio-afhankelijk parsen/tonen, wat bij opslaan-als-CSV verminkte datums in AFAS opleverde; met echte cellen kan dat niet meer). Bedragen zijn échte getallen (centen/100, negatief voor credits), Tegenrekening/BTW blijven tekstcellen. **Tegenrekening/BTW-code volgen het BTW-tarief** (in het voorbeeld handmatig bijgeplakt, hier afgeleid): 21% → `8002`/`1`, 0%-ICL → `8018`/`34`; **9% is bewust leeg** — dat tarief komt in de stalen-app niet voor (live geverifieerd) en de code is onbekend, dus liever een zichtbaar gat dan een foute boeking. Pure opbouw + tests: `src/lib/afas-xlsx.ts` (`buildAfasXlsx`).
- **E-mail: ontvanger zichtbaar + kiesbaar (13-07):** de mail-route accepteert `to` (override; leeg = clientEmail-keten `orders.email_invoice → clients.email_invoice → orders.email → clients.contact_email`) en registreert `invoices.sent_to` naast `sent_at`. UI: mail-knoppen openen eerst een paneel met het vooringevulde, aanpasbare adres (InvoiceModal + CreditDialog). **Valkuil: één e-mailveld kan meerdere ontvangers bevatten** (HOME CENTER WOLVEGA: `zr-pdf@…, factuur@…`; NOVA PROJECT heeft een trailing komma). Graph weigert een komma-string als één adres — daarom splitst `src/lib/email-recipients.ts` op `,`/`;`/whitespace, valideert élk adres en geeft Graph een echte recipient-lijst (`GraphMailSendInput.to: string | string[]`). Nooit een rauw klant-e-mailveld als één adres doorgeven.
